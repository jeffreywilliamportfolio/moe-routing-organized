#!/usr/bin/env python3
"""Export per-token all-expert breakdowns for the 122B five-condition experience-probe run."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export per-token all-expert breakdowns for 122B five-condition experience-probe.")
...
def reduce_split(layer_probs: np.ndarray) -> dict[str, dict[str, np.ndarray]]:
    out = {"all": reduce_phase(layer_probs)}
    out["softmax"] = reduce_phase(layer_probs[SOFTMAX_LAYERS, :, :])
    out["deltanet"] = reduce_phase(layer_probs[DELTANET_LAYERS, :, :])
    return out


def token_meta_from_prompt(prompt_tokens: list[dict[str, Any]]) -> list[tuple[int, str]]:
    return [(int(tok["token_id"]), str(tok.get("piece", ""))) for tok in prompt_tokens]


def token_meta_from_generation(generated_tokens: list[dict[str, Any]], n_gen: int) -> list[tuple[int, str]]:
    return [(int(tok["token_id"]), str(tok.get("piece", ""))) for tok in generated_tokens[:n_gen]]
...
def write_summary(summary_path: Path, capture_dir: Path, rows: list[dict[str, Any]], highlight_expert: int) -> None:
    lines = [
        "# Per-Token Summary",
        "",
        f"- Capture dir: `{capture_dir}`",
        f"- Highlight expert: `E{highlight_expert}`",
        f"- Export format: one dense `.npz` and one readable `.tsv` per prompt",
        "",
    ...
    for row in rows:
        lines.append(
            f"| {row['prompt']} | {row['n_prompt']} | {row['n_gen']} | {row['n_gen_trim']} | "
            f"{row['e48_gen_w']:.6f} | {row['e48_softmax_gen_w']:.6f} | {row['e48_deltanet_gen_w']:.6f} | "
            f"{row['e48_gen_rank']} | {row['e48_softmax_rank']} |"
        )
...
def main() -> None:
    ...
    for cell_dir in sorted(p for p in capture_dir.iterdir() if p.is_dir()):
        ...
        prompt_tokens = load_token_file(cell_dir / "prompt_tokens.json")
        generated_tokens = load_token_file(cell_dir / "generated_tokens.json")

        prompt_meta = token_meta_from_prompt(prompt_tokens[:n_prompt])
        gen_meta = token_meta_from_generation(generated_tokens, min(n_gen, len(generated_tokens)))
        n_gen = len(gen_meta)
        ...
        summary_rows.append(
            {
                "prompt": cell_dir.name,
                "n_prompt": n_prompt,
                "n_gen": n_gen,
                "n_gen_trim": n_gen_trim,
                "e48_gen_w": float(pooled_gen[args.highlight_expert]),
                "e48_softmax_gen_w": float(pooled_soft[args.highlight_expert]),
                "e48_deltanet_gen_w": float(pooled_delta[args.highlight_expert]),
                "e48_gen_rank": rank_of(args.highlight_expert, pooled_gen),
                "e48_softmax_rank": rank_of(args.highlight_expert, pooled_soft),
            }
        )

    write_summary(out_dir / f"{capture_dir.name}_per_token_summary.md", capture_dir, summary_rows, args.highlight_expert)

...
# Import reconstruct_probs from the local bundle helper
THIS_DIR = Path(__file__).resolve().parent
QWEN_ROUTER = THIS_DIR / "qwen_router.py"
...
def load_generated_token_ids(cell_dir: Path) -> list[int]:
    ...
    try:
        ...
        # Some captures contain non-UTF8 token-piece payload bytes. We only need
        # token_id here, so fall back to a byte-level scan instead of failing.
...
def write_report(report_path: Path, capture_dir: Path, cells: list[dict], model_label: str):
    ...
    L.append(
        ...
        f"paragraphs, generation `-n 2048`, greedy seed 42, thinking suppressed "
        f"(`</think>\\n\\n`).\n\n"
    )

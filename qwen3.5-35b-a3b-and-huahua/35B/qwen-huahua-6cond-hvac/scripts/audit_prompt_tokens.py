...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suite", default="prompt-suite-v2.2.json", help="Path to prompt suite JSON.")
...
def write_prompt_tsv(records: list[dict[str, Any]], template: str, tsv_path: pathlib.Path) -> None:
    ...
    lines = [f"{r['id']}\t{wrap_text(r['text'], template)}" for r in records]
    tsv_path.write_text("\n".join(lines) + "\n")

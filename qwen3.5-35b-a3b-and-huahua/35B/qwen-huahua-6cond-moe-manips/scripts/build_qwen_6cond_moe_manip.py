...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the 6-condition MoE-manip runtime TSV.")
    parser.add_argument(
        "--thinking",
        action="store_true",
...
def default_out(thinking: bool) -> Path:
    if thinking:
        return EXPERIMENT_ROOT / "prompts" / "qwen-6cond-moe-manip-think.tsv"
    return OUT_TSV
...
def unwrap_prompt(serialized: str) -> str:
    ...
    matched_suffix = None
    for suffix in (chat_suffix(False), chat_suffix(True)):
        if body.endswith(suffix):
            matched_suffix = suffix
            break
    if matched_suffix is None:
        raise ValueError("unexpected prompt suffix")
    return body[len(CHAT_PREFIX) : -len(matched_suffix)]
...
def main() -> int:
    args = parse_args()
    out_tsv = args.out or default_out(args.thinking)
    suffix = chat_suffix(args.thinking)
